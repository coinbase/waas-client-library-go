...
#echo "Merge $BRANCH_TO_SYNC from $INTERNAL_REPO_URL to $PUBLIC_REPO_URL"
#	if ! git merge -m "Merge to sync between $INTERNAL_REPO_URL and $PUBLIC_REPO_URL" --log left/$BRANCH_TO_SYNC; then
#	>&2 echo "Fatal: unable to fetch from $PUBLIC_REPO_URL, rerun the script as soon as connnection restored."
#	exit 1
#fi
